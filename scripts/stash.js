      ...
      filesByChar[char].forEach((file) => {
        ...
        btn.onclick = () => {
          ...
          fetch(`https://cdn.jsdelivr.net/gh/bubbls/ugs-singlefile/UGS-Files/${encoded}?t=${Date.now()}`)
            ...
            .then((text) => {
                console.log("Got file content, attempting save...");
                
                fetch("http://localhost:3000/save", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ fileName: file, content: text }),
                })
                .then((res) => res.json())
                .then((data) => console.log("Save response:", data))
                .catch((err) => console.error("Save fetch error:", err));
            })
            .catch((err) => console.error("CDN fetch error:", err));
        };
        btn.style.width = "100%";
        btn.style.height = "100%";
        fetch(`/extras/${encodeURIComponent(file)}/game.html`, { method: "HEAD" })
            .then((r) => {
                if (r.ok) {
